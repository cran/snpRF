...
Modifications to deal with SNP data Greg Jenkins Dec. 16, 2014.
...
void classRF(double *autosome, double *xchrom, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
	     ...
	     int *testdat, double *xtsA, double *xtsX, int *xtsDim, int *clts, int *nts, 
             double *countts, int *outclts, int *labelts, double *proxts, double *errts,
             int *inbag) {
    /******************************************************************
     ...
     *  dimx:     three integers: number of variables in autosomes and
     *            X chromosome, and number of cases
    ...
    int nsample0, mdim, nclass, addClass, mtry, ntest, nsample, ndsize,
      ...
      keepInbag, nstrata, mdimX, mdimA, tdimA, tdimX, tdim;

    int jb, j, n, m, k, idxByNnode, idxByNsample, imp, localImp, iprox,
	oobprox, keepf, replace, stratify, trace, *nright,
      *nrightimp, *nout, *nclts, Ntree, ci, cj, cs, xj, tci, tcj, tcs, txj;
	...
	*jtr, *classFreq, *idmove, *jvr,
    ...
    x =          (double *) S_alloc(nsample*mdim, sizeof(double));
    xts =        (double *) S_alloc(ntest*tdim, sizeof(double));
    ...
    /*  moved this into loop since this will change based on which X chromosomes are selected */
    ...
    for (jb = 0; jb < Ntree; jb++) {
      ...
      /* randomly select a copy of x chromosome from individuals and combine with autosome  */
      
        for (ci = 0; ci < nsample; ci++) {
      
	  for (xj = 0; xj < mdimA; xj++) {

	    x[xj+(ci*mdim)] = autosome[xj+(ci*mdimA)];

	  }
	  
	  cs = (int) (unif_rand()*10)%2;
	    
	  for (cj = cs; cj < mdimX; cj+=2) {

	    x[xj+(ci*mdim)] = xchrom[cj+(ci*mdimX)];
	...
	/*
	 for (ci = 0; ci < nsample; ci++) {
	  ...
	  for (xj = 0; xj < mdimA; xj++) {
	    ...
	    Rprintf("%6.0f",autosome[xj+(ci*mdimA)]);
	    Rprintf(",");
	  ...
	  Rprintf("stop");
	  for (ci = 0; ci < nsample; ci++) {
	  for (cj = 0; cj < mdimX; cj++) {

	    Rprintf("%6.0f",xchrom[cj+(ci*mdimX)]);
	    Rprintf(",");
	...
	*/
	...
	/* now do the same with test set data if test set data exists */

	if(*testdat) {

        for (tci = 0; tci < ntest; tci++) {

	  for (txj = 0; txj < tdimA; txj++) {

	    xts[txj+(tci*tdim)] = xtsA[txj+(tci*tdimA)];
	  ...
	  for (tcj = tcs; tcj < tdimX-1-tcs; tcj+=2) {

	    xts[txj+(tci*tdim)] = xtsX[tcj+(tci*tdimX)];
...
void classForest(int *dimx,double *autosome, double *xchrom, int *nclass, int *maxcat,
                 int *nrnodes, int *ntree, double *xbestsplit,
  ...
  int j, n, n1, n2, idxNodes, offset1, offset2, *junk, ntie, ci , xj, cj, cs, mdimA, mdimX, mdim, ntest;
  double crit, cmax, *x;
    ...
    x = (double *) S_alloc(ntest*mdim, sizeof(double));
    ...
    for (j = 0; j < *ntree; ++j) {
	  ...
	  for (cj = cs; cj < mdimX; cj+=2) {
       ...
       /*modified by Greg Jenkins, 12/16/2014 to deal with SNP data */
